...
parseRaw s = let codes = split "," s
  ...
  where
    str2IntPair s =
      let (h:values) = split " = " s
      in (read . filter isDigit $ h, read . head $ values)

results :: [Entry] -> Answer
results es = (foldr (+) 0 $ excute es Map.empty,
              foldr (+) 0 $ excute2 es Map.empty)
  where
    excute :: [Entry] -> Mem -> Mem
    excute [] m = m
    excute ((mask, codes):es) m =
      excute es $ foldl' (\m (addr, value) ->
                            Map.insert addr (mask `cover` value) m)
                         m
                         codes
    excute2 :: [Entry] -> Mem -> Mem
    excute2 [] m = m
    excute2 ((mask, codes):es) m =
      excute2 es $ foldl' (\m' (addr, value) ->
                             foldl' (\m'' addr ->
